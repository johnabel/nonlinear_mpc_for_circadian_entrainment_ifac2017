...
def mpc_problem_fig4(sampling_len):
    ...
    for idx,time in enumerate(time_steps[:-prediction_window]):
        ...
        mpc_ps+=list(sol[:,0])
        phases = [pmodel.phase_of_point(pt) for pt in sol[::int(60/sampling_len)]]
        tphases += list(tsol[::int(60/sampling_len)]+time)
    ...
    print str(sampling_len)+ ' completed'
...
with futures.ProcessPoolExecutor(max_workers=2) as executor:
    result = executor.map(mpc_problem_fig4, inputs, chunksize=1)
for idx,res in enumerate(result):
    subfig_results.append(res)

#np.save('Data/fig4b.npy',subfig_results[0])
np.save('Data/fig4c.npy',subfig_results[0])
'''

#load the results
sfr1 = np.load('Data/fig4b.npy')
sfr2 = np.load('Data/fig4c.npy')
...
plot_subfig_results(axes_set1, sfr1, shift=4, pred_win=12)
plot_subfig_results(axes_set2, sfr2, shift=4, pred_win=2)
